...
/**
 * Ends the command with exit code 1 by unwinding to the end of the switch below. Not
 * `process.exit`: a hard exit while a fetch's handles are still closing aborts inside libuv on
 * Windows (0xC0000409), as `refund` did whenever signerd refused it.
 */
...
try {
  switch (cmd) {
    ...
    case "refund": {
      ...
      // Only the header is needed. Left unread, the body holds the connection open to the end.
